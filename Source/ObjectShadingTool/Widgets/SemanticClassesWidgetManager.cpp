...
FReply FSemanticClassesWidgetManager::OnManageSemanticClassesClicked()
{
	...
	{
		UE_LOG(LogShadingTool, Error, TEXT("%s: Failed to load the main frame module"), *FString(__FUNCTION__))
	...
	{
		UE_LOG(LogShadingTool, Error, TEXT("%s: Failed to get the parent window"), *FString(__FUNCTION__))
		return FReply::Handled();
	}
...
{
	const bool bSuccess = ShadingManager->UpdateClassName(ClassName, NewText.ToString());
	if (bSuccess)
	{
		RefreshSemanticClasses();
	}
	...
	const FString ClassName)
{
	...
	FColorPickerArgs PickerArgs;
	{
		...
		PickerArgs.InitialColorOverride = FLinearColor(ShadingManager->ClassColor(ClassName));
	}
...
void FSemanticClassesWidgetManager::OnUpdateClassColorCommited(const FLinearColor NewLinearColor)
{
	...
	ShadingManager->UpdateClassColor(CurrentlyEditedClass, NewColor);
...
{
	const bool bSuccess = ShadingManager->RemoveSemanticClass(ClassName);
	if (bSuccess)
	{
		RefreshSemanticClasses();
	}
...
{
	const bool bSuccess = ShadingManager->NewSemanticClass(NewClassName.ToString(), NewClassColor);
	if (bSuccess)
	{
		NewClassName = FText::GetEmpty();
		NewClassColor = FColor::White;
...
FReply FSemanticClassesWidgetManager::OnDoneClicked()
{
	...
	{
		UE_LOG(LogShadingTool, Error, TEXT("%s: Widget window is invalid"), *FString(__FUNCTION__))
	}
...
void FSemanticClassesWidgetManager::RefreshSemanticClasses()
{
	...
	{
		UE_LOG(LogShadingTool, Error, TEXT("%s: Invalid classes box"), *FString(__FUNCTION__))
		return;
	}

	ClassesBox.Pin()->ClearChildren();

	TArray<const FShadingSemantic*> SemanticClasses = ShadingManager->GetSemanticClasses();
	for (int i = 0; i < SemanticClasses.Num(); i++)
	{
		const FShadingSemantic* SemanticClass = SemanticClasses[i];
